...
(function (Anterec) {
    var JsDragTable = (function () {
        function JsDragTable(target, options) {
        ...
        JsDragTable.prototype.rebind = function () {
            ...
            $(this.container).find("th").each(function (headerIndex, header) {
                if(_this.options.skipFirst && headerIndex == 0){
                    return;
                }
                $(header).off("mousedown touchstart");
                $(header).off("mouseup touchend");
                $(header).on("mousedown touchstart", function (event) {
                    if(_this.options.handleElementName != undefined){
                        if(!$(event.toElement).is(_this.options.handleElementName)){
                            _this.cancelColumn();
                            return;
                        }
                    }
        ...
        JsDragTable.prototype.dropColumn = function (header, event) {
            ...
            if(this.options.skipFirst && targetIndex == 0){
                return;
            }
        ...
        JsDragTable.prototype.dropEvent = function () {
            if(this.options.dropEvent != undefined){
                this.options.dropEvent(this.container);
            }
        }
...
jQuery.fn.extend({
    jsdragtable: function (options) {
        var defaults = {
            skipFirst: true
        };
        options = $.extend(defaults, options);

        return new Anterec.JsDragTable(this, options);
    }
});